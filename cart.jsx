...
const Products = (props) => {
  ...
  const addToCart = (e) => {
    ...
    if (item && item.instock > 0) {
      setCart((prevCart) => {
        ...
        let existingItem = prevCart.find((prod) => prod.name === item.name);
        if (existingItem) {
          return prevCart.map((prod) =>
            prod.name === item.name ? { ...prod, quantity: prod.quantity + 1 } : prod
          );
        } else {
      ...
      setItems(
        items.map((prod) => {
          if (prod.name === name && prod.instock > 0) {
            prod.instock -= 1;
          }
          return prod;
        })
  ...
  const deleteCartItem = (index) => {
    ...
    setItems(
      items.map((prod) => {
        if (prod.name === itemToRemove.name) {
          prod.instock += itemToRemove.quantity;
        }
        return prod;
      })
    );
  ...
  let cartList = cart.map((item, index) => {
    return (
      <Card key={index}>
        ...
        <Accordion.Collapse
          onClick={() => {
            deleteCartItem(index); // Remove from cart and update stock
          }}
          eventKey={1 + index}
        >
          <Card.Body>
  ...
  const checkOut = () => {
    // Suma total de todos los items en el carro
  ...
  const restockProducts = (url) => {
    ...
    setItems(data.data.map((product) => ({
